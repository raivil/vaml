...
namespace :vaml do
  task :add_secret, [:arg1, :arg2] do |t, args|
    ...
    unless key && value
      puts "Usage: VAULT_HOST=xxx VAULT_TOKEN=xxx rake vaml:add_secret /secret/data/development/xxx value"
      raise
    end
    Vaml.configure(host: ENV['VAULT_HOST'], token: ENV['VAULT_TOKEN'])
    Vaml.write_secret_string(key, value)
  ...
  task :read_secret do
    Vaml.configure(host: ENV['VAULT_HOST'], token: ENV['VAULT_TOKEN'])
    puts Vaml.read_secret_string(ARGV[1])
    exit
  end
end
